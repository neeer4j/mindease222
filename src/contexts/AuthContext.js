...
export const AuthProvider = ({ children }) => {
    ...
    const handleFirebaseError = (err) => {
        ...
        const errorMap = {
            ...
            "auth/invalid-credential": "Invalid email or password. If you signed up with Google, please use the Google sign-in method.",
    ...
    useEffect(() => {
        const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
            ...
            if (firebaseUser) {
                ...
                // No user initially; process potential redirect result
    ...
    const signup = async (email, password, displayName) => {
        ...
        try {
            ...
            // Note: Fixed setDoc call – removed extra parameter.
    ...
    // Sign in with Google – UPDATED TO ALWAYS USE POPUP, even on mobile.
    const signInWithGoogle = async () => {
        setLoading(true);
        setError(null);
        setSuccess(null);
        try {
            // Always use popup, regardless of device.
    ...
    const deleteAvatar = async () => {
        ...
        setSuccess(null);
        try {
            const storagePath = user.avatar.split('.com/o/')[1].split('?alt=media')[0];
    ...
    return (
        <AuthContext.Provider
            value={{
                ...
                setError // Expose setError function, you already had this actually, good.
            }}
        >
            {children}
        </AuthContext.Provider>
